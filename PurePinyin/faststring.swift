...
//        self.buffer.baseAddress!.deallocateCapacity(self.buffer.count + 1)
    ...
    public func toString() -> String {
        return String.init(cString: UnsafePointer<UInt8>(self.buffer.baseAddress!))
/*        if let result = String.init(cString: UnsafePointer<UInt8>(self.buffer.baseAddress!)) {
            return result
        }
        return nil*/
...
extension FastString: CustomStringConvertible {
    public var description: String {
        return self.buffer.baseAddress!.debugDescription
